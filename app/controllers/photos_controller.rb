        ...
        if @photo.save
          ...
          if @photo.user.me2day_connected? == true and params[:photo][:post_to_me2day] == "yes"
            ...
            if JSON.parse(@client.noop)["code"].to_s == "0" #"성공했습니다."
              result = @client.create_post @photo.user.me2day_nickname, 'post[body]' => "#{truncate(@photo.title, :length => 120)} #{shortened_url(@photo)}", 'attachment' => File.open(Rails.root.to_s+"/public"+@photo.image.url.split("?")[0])
